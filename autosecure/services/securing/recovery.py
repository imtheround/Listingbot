"""Recovery code secure flow for Microsoft accounts."""
...
log = get_logger("securing.recovery")
...
async def recovery_secure(
    ...
    """Secure an account using a recovery code.

    Uses the recovery code for initial login, then performs post-login
    security actions to fully secure the account.
    ...
    """
    log.info("recovery_secure.start", email=email, user_id=user_id)
    ...
    try:
        ...
        if not result.success:
            log.warning(
                "recovery_secure.login_failed", email=email, error=result.error
            )
            return SecureResult(
                success=False,
        ...
        if user_id and db:
            await _log_audit(user_id, "recovery_secure", account_data, True, db)

        log.info(
            "recovery_secure.success",
            email=email,
        ...
        log.warning("recovery_secure.invalid_credentials", email=email, error=str(e))
        return SecureResult(success=False, error=str(e), method="recovery")
    except RateLimited as e:
        log.warning("recovery_secure.rate_limited", email=email, error=str(e))
        return SecureResult(success=False, error=str(e), method="recovery")
    except AutoSecureError as e:
        log.error("recovery_secure.error", email=email, error=str(e))
        return SecureResult(success=False, error=str(e), method="recovery")
    except Exception as e:
        log.error("recovery_secure.unexpected_error", email=email, error=str(e))
        return SecureResult(success=False, error=f"Unexpected error: {e}", method="recovery")
...
async def _extract_account_data(
    ...
    """Extract account information from recovery login session.
    ...
    Args:
        ...
        recovery_code: The recovery code used.
...
async def _log_audit(
    ...
    try:
        ...
        log.error("recovery_secure.audit_log_failed", error=str(e))
