...
class appointSeller(commands.Cog):
    ...
    @discord.app_commands.command(name="checkowner", description="Check if a owner exists")
    @discord.app_commands.describe(human="human")
    async def checkOwner(self, interaction: discord.Interaction, human: discord.Member):
        await interaction.response.defer()
        if await self.db.check_owner(human.id):
            await interaction.followup.send(f"{human.mention} is an owner", ephemeral=True)
        else:
            await interaction.followup.send(f"{human.mention} is not an owner", ephemeral=True)
